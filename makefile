...
run: compile-source
	java -cp "$(BUILD_DIR)$(SEP)libs/*" $(MAIN_CLASS)

compile-tests:
	find tests -name '*.java' -print0 | xargs -0 javac -cp "src$(SEP)$(BUILD_DIR)$(SEP)libs/*" -d $(BUILD_DIR)

test: compile-tests
	java --enable-preview -cp $(BUILD_DIR)$(SEP)libs/* org.junit.runner.JUnitCore $$(find bin -name "*Test.class" -type f | sed 's@^bin/\(.*\)\.class$$@\1@' | sed 's@/@.@g')
